...
// NOTE: Runtime filesystem scanning of `public/product-images` (27k+ files,
// ~387 MB) was removed. It is never called at runtime (image URLs come from
// `input_pdfs_analysis_v5.json` / rendered-page data), but Next.js output file
// tracing (nft) statically follows any `fs.readdir(path.resolve(process.cwd(),
// 'public','product-images'))` and bundles the entire directory into every
// serverless function importing this module — pushing them past Vercel's 250 MB
// limit. Keeping this trigger out of the source is the durable fix; see also
// `outputFileTracingExcludes` in next.config.js as belt-and-suspenders.
